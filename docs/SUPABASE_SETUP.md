...
3. `0003_seed.sql` — first batch of verified content (see [CONTENT_MIGRATION.md](./CONTENT_MIGRATION.md))
4. `0004_page_content_upgrade.sql` — hero video controls on Site Settings, flexible `meta` column on
   content blocks
5. `0005_verified_content_upgrade.sql` — the full About page narrative, Yearling Preparation flyer copy,
   and the Soho Lanikai stallion

Then upload the real photos referenced by that content:

```bash
NEXT_PUBLIC_SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... node scripts/upload-seed-assets.mjs
```

(the three files in `supabase/seed-assets/` — see its own README for exactly what each one is)
