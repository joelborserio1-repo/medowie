...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!detail) return {};
  const { stallion } = detail;

  const name = stallionDisplayName(stallion.name, stallion.country_suffix);
  const description =
    stallion.meta_description ?? stallion.short_description ?? `${name} — Standardbred stallion standing at Medowie Lodge, NSW.`;

  return {
    title: stallion.meta_title ?? name,
    description,
    alternates: { canonical: `/stallions/${slug}` },
    openGraph: {
      title: name,
      description,
      images: stallion.hero_image_url ? [stallion.hero_image_url] : undefined,
    },
  };
}
